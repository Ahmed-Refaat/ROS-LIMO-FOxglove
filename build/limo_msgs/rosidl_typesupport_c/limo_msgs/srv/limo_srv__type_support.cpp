// generated from rosidl_typesupport_c/resource/idl__type_support.cpp.em
// with input from limo_msgs:srv/LimoSrv.idl
// generated code does not contain a copyright notice

#include "cstddef"
#include "rosidl_runtime_c/message_type_support_struct.h"
#include "limo_msgs/srv/detail/limo_srv__struct.h"
#include "limo_msgs/srv/detail/limo_srv__type_support.h"
#include "limo_msgs/srv/detail/limo_srv__functions.h"
#include "rosidl_typesupport_c/identifier.h"
#include "rosidl_typesupport_c/message_type_support_dispatch.h"
#include "rosidl_typesupport_c/type_support_map.h"
#include "rosidl_typesupport_c/visibility_control.h"
#include "rosidl_typesupport_interface/macros.h"

namespace limo_msgs
{

namespace srv
{

namespace rosidl_typesupport_c
{

typedef struct _LimoSrv_Request_type_support_ids_t
{
  const char * typesupport_identifier[2];
} _LimoSrv_Request_type_support_ids_t;

static const _LimoSrv_Request_type_support_ids_t _LimoSrv_Request_message_typesupport_ids = {
  {
    "rosidl_typesupport_fastrtps_c",  // ::rosidl_typesupport_fastrtps_c::typesupport_identifier,
    "rosidl_typesupport_introspection_c",  // ::rosidl_typesupport_introspection_c::typesupport_identifier,
  }
};

typedef struct _LimoSrv_Request_type_support_symbol_names_t
{
  const char * symbol_name[2];
} _LimoSrv_Request_type_support_symbol_names_t;

#define STRINGIFY_(s) #s
#define STRINGIFY(s) STRINGIFY_(s)

static const _LimoSrv_Request_type_support_symbol_names_t _LimoSrv_Request_message_typesupport_symbol_names = {
  {
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, limo_msgs, srv, LimoSrv_Request)),
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_introspection_c, limo_msgs, srv, LimoSrv_Request)),
  }
};

typedef struct _LimoSrv_Request_type_support_data_t
{
  void * data[2];
} _LimoSrv_Request_type_support_data_t;

static _LimoSrv_Request_type_support_data_t _LimoSrv_Request_message_typesupport_data = {
  {
    0,  // will store the shared library later
    0,  // will store the shared library later
  }
};

static const type_support_map_t _LimoSrv_Request_message_typesupport_map = {
  2,
  "limo_msgs",
  &_LimoSrv_Request_message_typesupport_ids.typesupport_identifier[0],
  &_LimoSrv_Request_message_typesupport_symbol_names.symbol_name[0],
  &_LimoSrv_Request_message_typesupport_data.data[0],
};

static const rosidl_message_type_support_t LimoSrv_Request_message_type_support_handle = {
  rosidl_typesupport_c__typesupport_identifier,
  reinterpret_cast<const type_support_map_t *>(&_LimoSrv_Request_message_typesupport_map),
  rosidl_typesupport_c__get_message_typesupport_handle_function,
  &limo_msgs__srv__LimoSrv_Request__get_type_hash,
  &limo_msgs__srv__LimoSrv_Request__get_type_description,
  &limo_msgs__srv__LimoSrv_Request__get_type_description_sources,
};

}  // namespace rosidl_typesupport_c

}  // namespace srv

}  // namespace limo_msgs

#ifdef __cplusplus
extern "C"
{
#endif

const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_c, limo_msgs, srv, LimoSrv_Request)() {
  return &::limo_msgs::srv::rosidl_typesupport_c::LimoSrv_Request_message_type_support_handle;
}

#ifdef __cplusplus
}
#endif

// already included above
// #include "cstddef"
// already included above
// #include "rosidl_runtime_c/message_type_support_struct.h"
// already included above
// #include "limo_msgs/srv/detail/limo_srv__struct.h"
// already included above
// #include "limo_msgs/srv/detail/limo_srv__type_support.h"
// already included above
// #include "limo_msgs/srv/detail/limo_srv__functions.h"
// already included above
// #include "rosidl_typesupport_c/identifier.h"
// already included above
// #include "rosidl_typesupport_c/message_type_support_dispatch.h"
// already included above
// #include "rosidl_typesupport_c/type_support_map.h"
// already included above
// #include "rosidl_typesupport_c/visibility_control.h"
// already included above
// #include "rosidl_typesupport_interface/macros.h"

namespace limo_msgs
{

namespace srv
{

namespace rosidl_typesupport_c
{

typedef struct _LimoSrv_Response_type_support_ids_t
{
  const char * typesupport_identifier[2];
} _LimoSrv_Response_type_support_ids_t;

static const _LimoSrv_Response_type_support_ids_t _LimoSrv_Response_message_typesupport_ids = {
  {
    "rosidl_typesupport_fastrtps_c",  // ::rosidl_typesupport_fastrtps_c::typesupport_identifier,
    "rosidl_typesupport_introspection_c",  // ::rosidl_typesupport_introspection_c::typesupport_identifier,
  }
};

typedef struct _LimoSrv_Response_type_support_symbol_names_t
{
  const char * symbol_name[2];
} _LimoSrv_Response_type_support_symbol_names_t;

#define STRINGIFY_(s) #s
#define STRINGIFY(s) STRINGIFY_(s)

static const _LimoSrv_Response_type_support_symbol_names_t _LimoSrv_Response_message_typesupport_symbol_names = {
  {
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, limo_msgs, srv, LimoSrv_Response)),
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_introspection_c, limo_msgs, srv, LimoSrv_Response)),
  }
};

typedef struct _LimoSrv_Response_type_support_data_t
{
  void * data[2];
} _LimoSrv_Response_type_support_data_t;

static _LimoSrv_Response_type_support_data_t _LimoSrv_Response_message_typesupport_data = {
  {
    0,  // will store the shared library later
    0,  // will store the shared library later
  }
};

static const type_support_map_t _LimoSrv_Response_message_typesupport_map = {
  2,
  "limo_msgs",
  &_LimoSrv_Response_message_typesupport_ids.typesupport_identifier[0],
  &_LimoSrv_Response_message_typesupport_symbol_names.symbol_name[0],
  &_LimoSrv_Response_message_typesupport_data.data[0],
};

static const rosidl_message_type_support_t LimoSrv_Response_message_type_support_handle = {
  rosidl_typesupport_c__typesupport_identifier,
  reinterpret_cast<const type_support_map_t *>(&_LimoSrv_Response_message_typesupport_map),
  rosidl_typesupport_c__get_message_typesupport_handle_function,
  &limo_msgs__srv__LimoSrv_Response__get_type_hash,
  &limo_msgs__srv__LimoSrv_Response__get_type_description,
  &limo_msgs__srv__LimoSrv_Response__get_type_description_sources,
};

}  // namespace rosidl_typesupport_c

}  // namespace srv

}  // namespace limo_msgs

#ifdef __cplusplus
extern "C"
{
#endif

const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_c, limo_msgs, srv, LimoSrv_Response)() {
  return &::limo_msgs::srv::rosidl_typesupport_c::LimoSrv_Response_message_type_support_handle;
}

#ifdef __cplusplus
}
#endif

// already included above
// #include "cstddef"
// already included above
// #include "rosidl_runtime_c/message_type_support_struct.h"
// already included above
// #include "limo_msgs/srv/detail/limo_srv__struct.h"
// already included above
// #include "limo_msgs/srv/detail/limo_srv__type_support.h"
// already included above
// #include "limo_msgs/srv/detail/limo_srv__functions.h"
// already included above
// #include "rosidl_typesupport_c/identifier.h"
// already included above
// #include "rosidl_typesupport_c/message_type_support_dispatch.h"
// already included above
// #include "rosidl_typesupport_c/type_support_map.h"
// already included above
// #include "rosidl_typesupport_c/visibility_control.h"
// already included above
// #include "rosidl_typesupport_interface/macros.h"

namespace limo_msgs
{

namespace srv
{

namespace rosidl_typesupport_c
{

typedef struct _LimoSrv_Event_type_support_ids_t
{
  const char * typesupport_identifier[2];
} _LimoSrv_Event_type_support_ids_t;

static const _LimoSrv_Event_type_support_ids_t _LimoSrv_Event_message_typesupport_ids = {
  {
    "rosidl_typesupport_fastrtps_c",  // ::rosidl_typesupport_fastrtps_c::typesupport_identifier,
    "rosidl_typesupport_introspection_c",  // ::rosidl_typesupport_introspection_c::typesupport_identifier,
  }
};

typedef struct _LimoSrv_Event_type_support_symbol_names_t
{
  const char * symbol_name[2];
} _LimoSrv_Event_type_support_symbol_names_t;

#define STRINGIFY_(s) #s
#define STRINGIFY(s) STRINGIFY_(s)

static const _LimoSrv_Event_type_support_symbol_names_t _LimoSrv_Event_message_typesupport_symbol_names = {
  {
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, limo_msgs, srv, LimoSrv_Event)),
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_introspection_c, limo_msgs, srv, LimoSrv_Event)),
  }
};

typedef struct _LimoSrv_Event_type_support_data_t
{
  void * data[2];
} _LimoSrv_Event_type_support_data_t;

static _LimoSrv_Event_type_support_data_t _LimoSrv_Event_message_typesupport_data = {
  {
    0,  // will store the shared library later
    0,  // will store the shared library later
  }
};

static const type_support_map_t _LimoSrv_Event_message_typesupport_map = {
  2,
  "limo_msgs",
  &_LimoSrv_Event_message_typesupport_ids.typesupport_identifier[0],
  &_LimoSrv_Event_message_typesupport_symbol_names.symbol_name[0],
  &_LimoSrv_Event_message_typesupport_data.data[0],
};

static const rosidl_message_type_support_t LimoSrv_Event_message_type_support_handle = {
  rosidl_typesupport_c__typesupport_identifier,
  reinterpret_cast<const type_support_map_t *>(&_LimoSrv_Event_message_typesupport_map),
  rosidl_typesupport_c__get_message_typesupport_handle_function,
  &limo_msgs__srv__LimoSrv_Event__get_type_hash,
  &limo_msgs__srv__LimoSrv_Event__get_type_description,
  &limo_msgs__srv__LimoSrv_Event__get_type_description_sources,
};

}  // namespace rosidl_typesupport_c

}  // namespace srv

}  // namespace limo_msgs

#ifdef __cplusplus
extern "C"
{
#endif

const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_c, limo_msgs, srv, LimoSrv_Event)() {
  return &::limo_msgs::srv::rosidl_typesupport_c::LimoSrv_Event_message_type_support_handle;
}

#ifdef __cplusplus
}
#endif

// already included above
// #include "cstddef"
#include "rosidl_runtime_c/service_type_support_struct.h"
// already included above
// #include "limo_msgs/srv/detail/limo_srv__type_support.h"
// already included above
// #include "rosidl_typesupport_c/identifier.h"
#include "rosidl_typesupport_c/service_type_support_dispatch.h"
// already included above
// #include "rosidl_typesupport_c/type_support_map.h"
// already included above
// #include "rosidl_typesupport_interface/macros.h"
#include "service_msgs/msg/service_event_info.h"
#include "builtin_interfaces/msg/time.h"

namespace limo_msgs
{

namespace srv
{

namespace rosidl_typesupport_c
{
typedef struct _LimoSrv_type_support_ids_t
{
  const char * typesupport_identifier[2];
} _LimoSrv_type_support_ids_t;

static const _LimoSrv_type_support_ids_t _LimoSrv_service_typesupport_ids = {
  {
    "rosidl_typesupport_fastrtps_c",  // ::rosidl_typesupport_fastrtps_c::typesupport_identifier,
    "rosidl_typesupport_introspection_c",  // ::rosidl_typesupport_introspection_c::typesupport_identifier,
  }
};

typedef struct _LimoSrv_type_support_symbol_names_t
{
  const char * symbol_name[2];
} _LimoSrv_type_support_symbol_names_t;

#define STRINGIFY_(s) #s
#define STRINGIFY(s) STRINGIFY_(s)

static const _LimoSrv_type_support_symbol_names_t _LimoSrv_service_typesupport_symbol_names = {
  {
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, limo_msgs, srv, LimoSrv)),
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_SYMBOL_NAME(rosidl_typesupport_introspection_c, limo_msgs, srv, LimoSrv)),
  }
};

typedef struct _LimoSrv_type_support_data_t
{
  void * data[2];
} _LimoSrv_type_support_data_t;

static _LimoSrv_type_support_data_t _LimoSrv_service_typesupport_data = {
  {
    0,  // will store the shared library later
    0,  // will store the shared library later
  }
};

static const type_support_map_t _LimoSrv_service_typesupport_map = {
  2,
  "limo_msgs",
  &_LimoSrv_service_typesupport_ids.typesupport_identifier[0],
  &_LimoSrv_service_typesupport_symbol_names.symbol_name[0],
  &_LimoSrv_service_typesupport_data.data[0],
};

static const rosidl_service_type_support_t LimoSrv_service_type_support_handle = {
  rosidl_typesupport_c__typesupport_identifier,
  reinterpret_cast<const type_support_map_t *>(&_LimoSrv_service_typesupport_map),
  rosidl_typesupport_c__get_service_typesupport_handle_function,
  &LimoSrv_Request_message_type_support_handle,
  &LimoSrv_Response_message_type_support_handle,
  &LimoSrv_Event_message_type_support_handle,
  ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_CREATE_EVENT_MESSAGE_SYMBOL_NAME(
    rosidl_typesupport_c,
    limo_msgs,
    srv,
    LimoSrv
  ),
  ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_DESTROY_EVENT_MESSAGE_SYMBOL_NAME(
    rosidl_typesupport_c,
    limo_msgs,
    srv,
    LimoSrv
  ),
  &limo_msgs__srv__LimoSrv__get_type_hash,
  &limo_msgs__srv__LimoSrv__get_type_description,
  &limo_msgs__srv__LimoSrv__get_type_description_sources,
};

}  // namespace rosidl_typesupport_c

}  // namespace srv

}  // namespace limo_msgs

#ifdef __cplusplus
extern "C"
{
#endif

const rosidl_service_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_SYMBOL_NAME(rosidl_typesupport_c, limo_msgs, srv, LimoSrv)() {
  return &::limo_msgs::srv::rosidl_typesupport_c::LimoSrv_service_type_support_handle;
}

#ifdef __cplusplus
}
#endif
